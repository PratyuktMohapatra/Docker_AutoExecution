...
def process_container(number):
    ...
    try:
        ...
        # Save HAR data after the application flow finishes
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        har_file_path = os.path.join(LOCAL_LOG_FOLDER, f"har_data_{timestamp}.har")
        subprocess.run(f"docker cp {container_name}:/root/Logs/har_data {har_file_path}", shell=True, check=True)
        logging.info(f"Saved HAR data to {har_file_path}")
